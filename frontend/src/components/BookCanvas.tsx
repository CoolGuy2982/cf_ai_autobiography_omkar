...
export const BookCanvas: React.FC<BookCanvasProps> = ({ content, chapterTitle, visible, isGenerating }) => {
    ...
    useEffect(() => {
        ...
        // CONSERVATIVE LIMIT: 550 chars (~100 words) prevents overflow on almost all screens
        const CHARS_PER_PAGE = 550; 

        paragraphs.forEach(para => {
            // A. FORCED PAGE BREAK FOR CHAPTERS
            // Matches "# Chapter", "## Chapter", or just "Chapter X:"
            const isChapterHeader = /^(#+\s*)?Chapter\s+\d+/i.test(para);

            if (isChapterHeader) {
                // If we have content on the current page, push it and start fresh
                if (currentPageText.trim().length > 0) {
                    generatedPages.push(currentPageText);
                    currentPageText = "";
                }
                // The chapter header becomes the start of the new page
            ...
            if ((currentPageText.length + para.length) > CHARS_PER_PAGE) {
                // If the paragraph itself is massive (larger than a page), split it
                if (para.length > CHARS_PER_PAGE) {
                    ...
                    // Normal overflow: push current page, start new one with this paragraph
                    generatedPages.push(currentPageText);
                    currentPageText = para + "\n\n";
                }
            } else {
    ...
    // === AUTO-FLIP ON GENERATION ===
    useEffect(() => {
        if (isGenerating && content.length > prevContentLength.current) {
            const lastSpreadIndex = pages.length % 2 === 0 ? pages.length - 2 : pages.length - 1;
            if (currentSpreadIndex < lastSpreadIndex) {
                 const timer = setTimeout(() => {
                     if (!isFlipping) {
                        setFlipDirection('next');
                        setIsFlipping(true);
                        setCurrentSpreadIndex(lastSpreadIndex);
    ...
    }, [content.length, isGenerating, pages.length, currentSpreadIndex, isFlipping]);
    ...
    const handleFlip = (dir: 'next' | 'prev') => {
        ...
        if (dir === 'next') {
            if (currentSpreadIndex < pages.length - 2) {
                setIsFlipping(true);
                setFlipDirection('next');
                setTimeout(() => {
                    setCurrentSpreadIndex(p => p === -1 ? 0 : p + 2);
                    setIsFlipping(false);
                }, 900);
            }
        }
    ...
    const toggleBookmark = () => {
        ...
        setBookmarkedSpreads(prev => {
            ...
            if (next.has(currentSpreadIndex)) {
                next.delete(currentSpreadIndex);
            } else {
                next.add(currentSpreadIndex);
            }
            return next;
        });
    };
                ...
                if (match) {
                    return (
                        <div className="mb-10 text-center mt-4">
                            <span className="block font-sans text-xs font-bold uppercase tracking-[0.3em] text-stone-500 mb-2">
                                Chapter {match[1]}
                            </span>
                            <h1 className="text-3xl font-serif font-bold text-[#2c1810] leading-tight">
                                {match[2]}
                            </h1>
    ...
    return (
        ...
        >
            <motion.div 
                ...
                initial={false}
                animate={{ 
                    width: isCover ? 480 : 960,
                    rotateX: isCover ? 0 : 5
                }}
            >
                {/* 1. BACK COVER */}
                <div className={cn(
                    "absolute bg-[#1a0f0a] rounded-[6px] -z-20 transform translate-y-3 translate-x-2 shadow-2xl transition-all duration-1000",
                    isCover ? "-inset-1" : "-inset-2"
                )}>
                ...
                {/* === LEFT SIDE === */}
                <div className={cn(
                    "h-full relative overflow-hidden transition-all duration-1000 origin-right bg-[#f4ecd8] border-r border-[#dcd1b9] rounded-l-[3px]",
                    isCover ? "w-0 opacity-0 border-none" : "w-1/2 opacity-100"
                )}>
                    ...
                    {/* Content Container - z-30 ensures it's clickable/scrollable above background layers */}
                    ...
                    {!isFlipping && !isCover && (
                        <div 
                            ...
                            className="absolute top-0 bottom-0 left-0 w-24 z-50 cursor-pointer group flex items-end justify-start pl-6 pb-8"
                        >
                            <span className="font-serif italic text-stone-400/60 opacity-0 group-hover:opacity-100 transition-opacity duration-500 transform -translate-x-2 group-hover:translate-x-0">
                                Previous Page
                            </span>
                        </div>
                    )}
                ...
                {/* === RIGHT SIDE === */}
                <div className={cn(
                    "h-full relative overflow-hidden bg-[#f4ecd8] rounded-r-[3px] transition-all duration-1000",
                    isCover ? "w-full rounded-[4px]" : "w-1/2"
                )}>
                    {isCover ? (
                        /* FRONT COVER */
                        <div className="absolute inset-0 z-30 flex flex-col items-center justify-center text-center p-12 bg-[#1c1917] overflow-hidden rounded-[4px] shadow-inner">
                            ...
                            <div className="relative z-10 border-[1px] border-orange-500/30 p-1 w-full h-full flex flex-col items-center justify-center">
                                <div className="border-[2px] border-orange-600/20 w-full h-full flex flex-col items-center justify-center p-8">
                                    <h1 className="text-5xl font-serif font-bold text-[#e7e5e4] tracking-wide mb-6 drop-shadow-2xl">
                                        {chapterTitle}
                                    </h1>
                                    ...
                                    <button 
                                        ...
                                        className="mt-24 group relative px-8 py-3 bg-transparent border border-orange-700/50 text-orange-600 font-sans font-bold uppercase tracking-wider text-[10px] hover:bg-orange-900/20 transition-all cursor-pointer"
                                    >
                                        Open Book
                        ...
                        <>
                            ...
                            {!isCurrentBookmarked && (
                                <div className="absolute top-6 right-6 z-40">
                                    <button 
                                        ...
                                        className="text-stone-400 hover:text-orange-600 transition-colors duration-300"
                            ...
                            <AnimatePresence>
                                {isCurrentBookmarked && (
                                    <motion.div
                                        ...
                                        onClick={toggleBookmark}
                                        className="absolute top-0 right-8 w-8 bg-orange-600 shadow-md z-40 origin-top cursor-pointer hover:bg-orange-700 transition-colors"
                            ...
                            {!isFlipping && currentSpreadIndex < pages.length - 2 && (
                                <div 
                                    onClick={() => handleFlip('next')}
                                    className="absolute bottom-0 right-0 w-32 h-32 z-50 cursor-pointer group overflow-hidden"
                                >
                                    <div 
                                        className="absolute bottom-0 right-0 w-full h-full bg-gradient-to-tl from-[#dbd2c2] to-[#e3dacb] shadow-[-3px_-3px_10px_rgba(0,0,0,0.15)] transition-all duration-300 ease-out origin-bottom-right scale-0 opacity-0 group-hover:scale-100 group-hover:opacity-100"
                ...
                <AnimatePresence custom={flipDirection} mode="popLayout">
                    {isFlipping && (
                        <motion.div
                            ...
                            animate="animate"
                            exit="exit"
                            className="absolute top-0 bottom-0 left-1/2 w-1/2 h-full bg-[#f4ecd8] origin-left z-40 rounded-r-[3px] backface-visible"
                            style={{ transformStyle: 'preserve-3d' }}
                        >
                            {/* Front of Flip */}
                            <div className={cn(
                                "absolute inset-0 backface-hidden z-20 overflow-hidden",
                                isCover && flipDirection === 'next' ? "bg-[#1c1917]" : "bg-[#f4ecd8] rounded-r-[3px]"
                            )}>
                                {isCover && flipDirection === 'next' ? (
                                    /* Cover Front Animating */
